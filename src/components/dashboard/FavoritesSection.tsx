...
// Hook to get properties data for dashboard
function useDashboardProperties() {
...
export function FavoritesSection() {
  ...
  const { properties, loading } = useDashboardProperties();
  const { addToast } = useToast();

  // Filter properties to show only favorites
  const favoriteProperties = properties.filter((property: any) => 
    user?.favorites.includes(property.id)
  );

  const handleRemoveFromFavorites = (propertyId: string) => {
    toggleFavorite(propertyId);
    addToast('Proprietà rimossa dai preferiti', 'success');
  };
